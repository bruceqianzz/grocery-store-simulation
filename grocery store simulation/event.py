...
class CustomerArrival(Event):
    ...
    _line_num: The number of the line this customer get in
    """
    customer: Customer
    _line_num: int
...
class CloseLine(Event):
    ...
    === Attributes ===
    line_number: The number of the checkout line.
    _name_list: Thr returned name list after the line is closed.
    """
    line_number: int
    _name_list: list
...
def create_event_list(event_file: TextIO) -> List[Event]:
    ...
    while line:
        ...
        if line[1] == "Close":
            list_event.append(CloseLine(int(line[0]), int(line[2])))
        else:
            items = []
            for i in range(2, 2 + ((len(line) - 3) // 2)):
                items.append(Item(line[2 * i - 1], int(line[2 * i])))
            list_event.append(CustomerArrival\
                                  (int(line[0]), Customer(line[2], items)))
        line = event_file.readline()
    return list_event
